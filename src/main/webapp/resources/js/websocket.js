      ...
      success:function(data){
         ...
         if(data.count>0){
            ...
            $.each(data.list,function(i,value){
               ...
               str+="<span class='pull-right'><i class='fa fa-clock-o'></i>"+value.regdate.substring(0,value.regdate.length-3)+"</span><br>";
               str+="<span class='pull-left'><i class='fa fa-user'></i>"+value.memberVO.id+"</span>";
               var title=value.title;
               if(title.length>5){
                  title=title.substr(0,5)+'...';
               }
               str+="<span class='pull-right'><i class='glyphicon glyphicon-comment'></i>"+title+"</span>";
               str+="</a>";
               str+="</div>";
