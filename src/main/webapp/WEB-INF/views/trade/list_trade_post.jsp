                  ...
                  <div class="list-img" onclick="location.href='detail_trade_post.do?pageNo=${requestScope.pageNo}&tradePostNo=${tpVO.tradePostNo}'">
                     ...
                     <div class="row">
                        ...
                        <div class="col-sm-4" style="padding-right: 31px;">
                        	<c:choose>
                        		<c:when test="${tpVO.status eq '거래대기'}">
                        			<div class="label label-info">${tpVO.status}</div>
                        		</c:when>
                        		<c:when test="${tpVO.status eq '거래중'}">
                        			<div class="label label-warning">${tpVO.status}</div>
                        		</c:when>
                        		<c:when test="${tpVO.status eq '거래완료'}">
                        			<div class="label label-danger">${tpVO.status}</div>
                        		</c:when>
                        		<c:otherwise>
                        			<div class="label label-danger">${requestScope.tradePostVO.status}</div>
                        		</c:otherwise>
                        	</c:choose>
                        </div>
                     </div>
